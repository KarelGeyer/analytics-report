...
const CategoriesByValue = () => {
    ...
    const saveProductNames = (e, res) => {
        formData.categoriesByQuantity.names.splice(res, 1, e.target.value)
    }

    const saveProductValues = (e, res) => {
        formData.categoriesByQuantity.values.splice(res, 1, e.target.value)
    }
    return (
        <>
            <h2> Kategorie podle kusů </h2>
            {dummy.map((res, i) => 
            ...
            <Link to="/addData/DataComparison">
